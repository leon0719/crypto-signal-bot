...
| 部位大小 | 沿用紙上規則:OKX 實際 USDT 權益 × 每筆風險 1%;槓桿依 ATR 動態 1x–5x(`risk.ts` `suggestLeverage`,與 2026-07-16 起的紙上帳本一致;固定風險下槓桿只影響保證金/強平價) |
...
2. **部位計算**:`GET /api/v5/account/balance` 取 USDT 權益 →
   ...
   以 `instruments` 的 `ctVal`/`lotSz`/`minSz` 換算並取整為合約張數;
   槓桿 = `suggestLeverage(ATR, entry)`(逐倉)。
