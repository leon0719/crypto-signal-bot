// LINE Messaging API:webhook 簽章驗證、回覆、載入動畫。

import type { LineMessage, QuickReply } from "./types.js";
...
// 用 reply token 回覆訊息陣列(text / flex,可帶 quickReply)。
...
// 顯示「輸入中…」載入動畫(只支援 1:1 聊天;loadingSeconds 須為 5 的倍數)。
export async function showLoading(accessToken: string, chatId: string, seconds = 5): Promise<void> {
  try {
...
export function textMessage(text: string, quickReply?: QuickReply): LineMessage {
  const msg: LineMessage = { type: "text", text: String(text).slice(0, MAX_TEXT_LENGTH) };
